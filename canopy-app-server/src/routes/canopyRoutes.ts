...
router.post("/create", async (req, res) => {
  try {
    ...
    const newCanopy = await prisma.canopy.create({
      data: {
        manage_number,
        class_number,
        location_id: location.id,
        buyer_id,
        status_fold: status_fold ?? false,
        status_motor: status_motor ?? false,
        status_led: status_led ?? false,
        status_sound: status_sound ?? false,
        status_inform: status_inform ?? false,
        status_temperature,
        status_transmit: status_transmit ?? false,
      },
    });
...
router.get("/show", async (req, res) => {
  try {
    ...
    const formattedRelatedData = canopies.reduce((acc: any, canopy) => {
      acc[canopy.id] = relatedData.filter(
        (rel) =>
